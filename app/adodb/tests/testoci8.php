...
/**	$rs = &$db->Execute(
	...
	*/
	...
	if (!$rs) die("Empty RS");
	if ($rs->EOF) die("EOF RS");
	rs2html($rs,"border=3 bgcolor='#effee'");
	
	$pager1 = new ADODB_Pager($db1,$sql,'adodb','true');
	$pager1->Render($rows_per_page=5,'width=100% border=1 bgcolor=#33ffff');
}
