...
function valida() {
	...
	<br>
	<form action="cons_desarrollo.php" method="post" name="form1">
	<?php
		$principal="cons_desarrollo.php";
	...
	<table border="0">
	<tr>
	  <td colspan="3">Proyectos por Estado de Desarrollo</td>
	</tr>
	<tr>
	  <td>	
	    <table>
	      ...
	      <tr>	    
		    ...
		    <td>			  
			  ...
			  ?>
			    >Todos(as)</option>
              <?php
		  	$sql="select estdes_codigo,estdes_nombre "
				."from estado_desarrollo "
				."order by estdes_codigo";
			$rs = &$conn->Execute($sql);
			while(!$rs->EOF)
			{
			  $valor=$rs->fields[0];
...
<?php	        
      if(isset($procesar))
	  {  
		$sql="select dp.datpro_id from dato_proyecto dp ";
		if($estado!="0")
		{
		  $sql.=",dp_datotecnico dpt "
		  	   ."where dpt.estdes_codigo=$estado and dp.datpro_id=dpt.datpro_id  ";
		}
		...
		<tr>
		  <td nowrap>
		    <SPAN class="title" STYLE="cursor:default;">
			  ...
			  <font color="#FFFFFF">Proyecto por Estado&nbsp;</font>
			</SPAN>
		  </td>
		</tr>
	  </table>
