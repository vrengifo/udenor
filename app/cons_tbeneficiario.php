...
function valida() {
	...
	<?php
		$principal="cons_tbeneficiario.php";
	...
	<table border="0">
	<tr>
	  <td colspan="3">Proyectos por Beneficiarios</td>
	</tr>
	<tr>
	  <td>	
	    <table>
	      <tr>	    
            <td colspan="2">
              Escoja la Opción deseada, si se ingresan los valores en Beneficiarios entre se hará la consulta entre rangos,
               si no se ingresó uno de los dos valores el sistema asume que escogió la segunda opción que es Beneficiarios por valor.  
               En caso de no ingresarse un valor para el Beneficiario por valor el sistema mostrará todos los datos que posee.
            </td>
	      </tr>
	      </tr>
	      <tr>	    
            <td>Beneficiarios Entre (Rangos): </td>
		    <td>			  
...
<?php	        
      if(isset($procesar))
	  {  
		$sql="select distinct datpro_id from beneficiosocial ";
		if((strlen($desde)>0)&&(strlen($hasta)>0))
		{
		  $sql.="where ben_directos>='$desde' and ben_directos<='$hasta' ";
		...
		else //opción
		{
		  if(strlen($valor)>0)
		  {
		  	$sql.="where ben_directos$operador'$valor' ";
		  }
		}
		$sql.="order by ben_directos desc";	
		$rs=&$conn->Execute($sql);
		...
		<tr>
		  <td nowrap>
		    <SPAN class="title" STYLE="cursor:default;">
			  ...
			  <font color="#FFFFFF">Proyecto por Beneficiarios&nbsp;</font>
			</SPAN>
		  </td>
		</tr>
	  </table>
