...
function valida() {
	...
	<table border="0">
	<tr>
	  <td colspan="3">Proyectos por Objetivos Udenor</td>
	</tr>
	<tr>
	  <td>	
	    <table>
	      <tr>	    
            <td>Objetivos Udenor: </td>
		    <td>			  
			  <select name="objetivo">
			    <option value="all" <?if($objetivo=="all") echo "selected";?>>Todos los objetivos</option>
			    <option value="none" <?if($objetivo=="none") echo "selected";?>>Ningún objetivo</option>
			    <?
			    $sql="select objude_codigo,objude_descripcion from objetivo_udenor order by objude_descripcion ";
			    $rs=&$conn->Execute($sql);
			    while(!$rs->EOF)
			    {
			      $valor=$rs->fields[0];
			      $texto=$rs->fields[1];
			    ?>
			    <option value="<?=$valor?>" <?if($objetivo==$valor) echo "selected";?>><?=$texto?></option>
			    <?
			      $rs->MoveNext();
			    }
			    ?>
...
<?php	        
      if(isset($procesar))
	  {  
		$sql="select distinct datpro_id from objxpri ";
		if(($objetivo!="all")&&($objetivo!="none"))
		{
		  $sql.="where objude_codigo='$objetivo' ";
		}
		else 
		{
		  if($objetivo=="none")
		  {
		  	$sql="select distinct cp.datpro_id from calificacion_proyecto cp,objxpri op "
		...
		<tr>
		  <td nowrap>
		    <SPAN class="title" STYLE="cursor:default;">
			  ...
			  <font color="#FFFFFF">Proyecto por Objetivos Udenor&nbsp;</font>
			</SPAN>
		  </td>
		</tr>
	  </table>
