...
/*
funcion que retorna una fecha a partir de una cantidad de horas
y una fecha origen
$horas es el nro de horas que se va a aumentar a la fecha origen
$fechaorigen es una cadena con formato YYYY-mm-dd
retorna una fecha con formato YYYY-mm-dd
*/
function hour2date($horas,$fechaorigen)
{
$dateact=explode("-",$fecha_origen);
$h2s=floor($horas*(60*60));//valor en segundos
$origendate=mktime("","","",$dateact[1],$dateact[2],$dateact[0]);
$nextdate=$h2s+$origendate;
$fecha=date("Y-m-d",$nextdate);
return $fecha;
}

/*
funcion que cambia de ciclos a horas, usado para los componentes
$ciclos es la cantidad de ciclos que se van a pasar a horas
$ceng_cac es la relacion entre ciclos de engine o assembly y ciclos del aircraft
$hac_cac es la relacion entre horas de aircraft y ciclos del aircraft
retorna un numero que es el nro de horas
*/
function cyc2hour($ciclos,$ceng_cac,$hac_cac)
{
  $valor=$ciclos*(1/$ceng_cac)*$hac_cac;
  return ($valor);
}

/*
funcion que cambia de horas a ciclos, usado para los componentes 
$horas es la cantidad de horas que se van a pasar a ciclos 
$heng_hac es la relacion entre horas de engine o assembly y horas del aircraft 
$hac_cac es la relacion entre horas de aircraft y ciclos del aircraft 
retorna un numero que es el nro de ciclos
*/
function hour2cyc($horas,$heng_hac,$hac_cac)
{
  $valor=$horas*(1/$heng_hac)*(1/$hac_cac);
  return ($valor);
}
