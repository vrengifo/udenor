...
$sql="select p.pro_nombre,c.can_nombre,par.par_nombre,dpu.ubi_comunidad,dpu.ubi_observacion "
		...
		else
		{
			...
			build_table($recordSet,false,$mainheader,'Ubicación','images/360/yearview.gif','100%',true);
		}	
?>
<hr width="100%" align="center" size="2">
<?	
...
//dp_entidades
		...
		else
		{
			...
			build_table($recordSet,false,$mainheader,'Entidades','images/360/yearview.gif','100%',true);
		}	
?>
<hr width="100%" align="center" size="2">
<?	
//dp_documentacion
		...
		else
		{
			...
			build_table($recordSet,false,$mainheader,'Documentación','images/360/yearview.gif','100%',true);
		}	
?>
<hr width="100%" align="center" size="2">
<?	
//calificar
	...
	$ccali=new c_calificacionProyecto($conn,$session_username);
	$secalifico=$ccali->calificar($obj->datpro_id,$msg);
	if($secalifico)
	{
	  $campo=array(
						array("etiqueta"=>" Concordancia del Proyecto","nombre"=>"clp0","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_concordanciaval),
						array("etiqueta"=>" Beneficiarios Directos","nombre"=>"clp1","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_beneficiosocialval),
						array("etiqueta"=>" Tasa Interna de Retorno","nombre"=>"clp2","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_benecotirval),
						array("etiqueta"=>" Valor Actual Neto","nombre"=>"clp3","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_benecovanval),
						array("etiqueta"=>" Gastos Administrativos","nombre"=>"clp4","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_benecogaval),
						array("etiqueta"=>" Vialidad Participativa","nombre"=>"clp5","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_vialidadval),
						array("etiqueta"=>" Contrapartida","nombre"=>"clp6","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_contrapartidaval),
						array("etiqueta"=>" Impacto Ambiental","nombre"=>"clp7","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->cal_ambientalval),
						array("etiqueta"=>" Total","nombre"=>"clp7","tipo_campo"=>"text","sql"=>"","valor"=>$ccali->total_puntaje." / ".$ccali->total_sobre)
				);
	}
	else 
	{
	  $campo=array(
						array("etiqueta"=>" Observación","nombre"=>"clp0","tipo_campo"=>"text","sql"=>"","valor"=>$msg)
				);		
	}
		...
		//construye el html para los campos relacionados
		build_show($conn,'false',"Calificación del Proyecto","images/360/taskwrite.gif","50%",'true'
		,$campo,$campo_hidden,$idp);
?>
  <a href="#" onclick="print();"><img src="images/360/print.gif" border="0" alt="Imprimir"></a>
  <input type="button" name="Cancel" value="Cerrar" onClick="window.close();">
